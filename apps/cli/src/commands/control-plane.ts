  ...
  if (!found) {
    ...
    log.error(
      'No GitHub credential found on this machine.\n' +
        ...
        'or use `agentbox hub setup --git-auth app` to create a GitHub App instead.',
    );
    return null;
  }
...
const setupSub = new Command('setup')
  .description(
    'Set up the control box\'s git credential and write its deploy config (env + admin token)',
  )
  .option(
    '--git-auth <mode>',
    "how the control box reaches GitHub: gh (default — reuse your own GitHub token, nothing to install) | app (create a GitHub App and lease per-repo tokens; needs the repo owner to install it)",
  )
  .option('--name <name>', 'GitHub App name (--git-auth app only; default: autogenerated agentbox-<rand>, must be globally unique)')
  .option('--org <org>', 'create the App under an organization instead of your user account (--git-auth app only)')
...
const promptsAnswerSub = new Command('answer')
  ...
  .argument('<id>', 'the prompt id from `hub prompts list`')
