...
agentbox hub setup --deploy vercel    # auto-forks via gh if you don't own the repo
...
The deployed hub is a full hub (Next.js + Postgres/SQLite) that holds the centralized concerns — git-token leasing via a GitHub App, permission state, the box registry/events — so cloud boxes keep pushing and opening PRs with your laptop off. `setup` creates the GitHub App, deploys the hub **from GitHub** (Vercel or a Hetzner VPS at `https://<ip>.sslip.io`; `--repo`/`--ref` choose the source, default `madarco/agentbox` at the ref matching your CLI — its `v<version>` tag, or `nightly` on a nightly build), and wires `relay.controlPlaneUrl`; `add` authorizes a repo on the App; `set-url` points boxes at an existing deployment; `boxes`/`prompts`/`jobs` drive + inspect it from the PC; `credentials`/`secrets`/`project`/`custody` seed its custody store; `worker` provisions boxes from the hub's queue. For Vercel, AgentBox auto-forks via `gh` (or the Deploy Button forks in-browser) so you don't need to own the repo; Hetzner clones it on the VPS so no ownership is needed either.
