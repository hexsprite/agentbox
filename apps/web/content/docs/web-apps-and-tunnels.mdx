...
The Portless proxy is a process, but the routes it serves are stored on disk — so after a restart the URLs still *exist* while nothing is listening on them. AgentBox restarts the proxy for you whenever it is about to hand out one of these URLs (creating a box, starting an agent, `agentbox hub`), and it does that silently, without asking for a password.

To have the proxy up from the moment you log in — before you run any AgentBox command — install Portless's startup service once:
...
The service runs the clean HTTPS proxy on port `443`, so box URLs become `https://<box-name>.localhost` with no port. `agentbox doctor` tells you which state you're in: proxy down, or running but with no startup service (fine until the next reboot).
