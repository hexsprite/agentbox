...
<Callout type="warn" title="Experimental">The deployed hub / control box is under active development. Its commands live under `agentbox hub` (setup/deploy/boxes/prompts/credentials/custody/…), self-documented via `agentbox hub --help`; expect rough edges. Local docker boxes are unaffected — this is only for cloud boxes.</Callout>
...
Two modes, set by [`hub.gitAuth`](/docs/configuration) or `agentbox hub setup --git-auth`:

| | `gh` (default) | `app` |
|---|---|---|
| Credential | your own GitHub token, held by the hub | a GitHub App private key, held by the hub |
| Boxes get | **nothing** — they ask the hub to push | a 1-hour, single-repo token, per push |
| Setup needs | `gh auth login` (you) | the repo **owner** to install the App |
| Works on | any repo you can push to | only repos the App is installed on |

In `gh` mode the box runs `git push`, the hub pulls the branch out of it as a git bundle, and the hub does the real push with its own token — the same path your laptop already uses for cloud boxes. A credential never enters the box or any snapshot of it.

`app` mode is tighter when you can get it: the token it hands out is scoped to one repo and expires in an hour. Choose it when you own the repos (or your org will install the App):

```bash
agentbox hub setup --git-auth app     # create a GitHub App instead
agentbox hub add                      # authorize a repo on it (app mode only)
```

<Callout type="info" title="Which token the hub holds">In `gh` mode the hub holds a normal user token, so it can reach every repo you can — broader than an App's per-repo token, though it never leaves the server. Prefer a **fine-grained** token limited to the repos you'll actually use; `hub setup` warns if the one it finds carries scopes like `admin:org` or `delete_repo`. Revoke it any time at [github.com/settings/tokens](https://github.com/settings/tokens) (or `gh auth logout`), then re-run `hub setup`.</Callout>
...
docker compose -f docker-compose.yml -f docker-compose.caddy.yml logs --tail=200 app
```

A **502** at `https://<ip>.sslip.io/healthz` means Caddy and the certificate are fine but the hub container isn't answering on `:8787` — check `logs app` above. A **certificate or connection** error instead means Caddy never came up. `cloud-init status --long` and `/var/log/cloud-init-output.log` cover the earlier phase (Docker install + repo clone).

Retry with `agentbox hub deploy hetzner` — it reuses the GitHub App and env from the first run, so there's no need to run `hub setup` again. The failed VPS and its firewall are still billing; delete them from the [Hetzner console](https://console.hetzner.cloud) (they're labeled `agentbox.role=control-plane`).

<Callout type="warn" title="Don't pin --ref to an older version">The deploy is a two-sided contract: the VPS builds `apps/hub` from the cloned ref, while your CLI generates the Caddy config and `.env` that wrap it. They only line up when both come from the same code, so `--ref` defaults to the ref matching your CLI — its `v<version>` tag on a release, `nightly` on a nightly build. Pin an older ref and the deploy stops before building, with an error naming `--ref`. Pass one only to deploy a **newer** branch (a feature branch you want to verify live).</Callout>
...
agentbox hub prompts list                # pending host-action approvals
agentbox hub prompts answer <id> y       # answer one (also answerable in the web UI)
...
UI, the menu-bar app, or `agentbox hub prompts answer`.
