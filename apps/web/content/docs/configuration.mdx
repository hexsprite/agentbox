...
[control box](/docs/deployed-hub), pointing at its VPS and the per-deploy key
...
| `update.channel` | enum | `auto` | release channel `self-update` and `install app` follow: `auto` follows the installed build, `nightly` opts into [pre-release builds](/docs/nightly), `stable` opts back out. Nightly installs the newest build of *either* channel, so a stable release supersedes the nightlies before it |
